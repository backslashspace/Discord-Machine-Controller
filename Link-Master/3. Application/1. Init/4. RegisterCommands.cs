...
namespace Link_Master.Worker
{
    internal static partial class Bot
    {
        ...
        {
            Log.FastLog("Initiator", "Initializing", xLogSeverity.Info);

            SocketGuild guild = Client.Discord.GetGuild((UInt64)CurrentConfig.GuildID);

            Log.FastLog("Initiator", "Deregistering commands", xLogSeverity.Info);
            await guild.DeleteApplicationCommandsAsync();

            Log.FastLog("Initiator", "Registering commands", xLogSeverity.Info);
            ...
            catch (HttpException exception)
            {
                ...
                Log.FastLog("Initiator", json, xLogSeverity.Critical);
            }
        }
    }
}